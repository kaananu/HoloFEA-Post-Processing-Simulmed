...
public class NastranImporter : MonoBehaviour
{
    public TextAsset ElemConnectivity;
    public TextAsset SpatialDisplacement;
    //public TextAsset ElemStressFront;
    //public TextAsset ElemStressBack;
    public TextAsset VMSAll;
    ...
    public void LoadModel()
    {
        ...
        string[] StressDataLines = VMSAll.text.Split(new char[] { '\r', '\n' }, StringSplitOptions.RemoveEmptyEntries);

        float[] StressAll = new float[StressDataLines.Length - 1];
        float[] StressFront = new float[StressDataLines.Length];
        float[] StressBack = new float[StressDataLines.Length];
        ...
        for (int i = 1; i < (StressDataLines.Length); i++)
        {
            ...
            StressAll[i - 1] = float.Parse(StressData[4]);
            StressFront[i1] = float.Parse(StressData[4]);
            i1 = i1 + 1;
            StressBack[i2] = float.Parse(StressData[4]);
            i2 = i2 + 1;
        }
        ...
        for (int i = 1; i < NodeDataLines.Length; i++)
        {
            ...
            float deltax = float.Parse(coord[5]);
            float deltay = float.Parse(coord[6]);
            float deltaz = float.Parse(coord[7]);
        ...
        //model1undeformed.AddComponent<MeshCombine>();
        model1undeformed.SetActive(false);

        GameObject scaleconfig2D = GameObject.Find("ColourScale");
        scaleconfig2D.GetComponent<ScaleConfig2D>().SetScaleValues(maxStress, minStress);

        //GameObject model1 = new GameObject("model1");
        //model1.transform.position = new Vector3(0, 0, 0);
        //model1.transform.parent = GameObject.Find("Model Space").transform;
        ...
        //model1undeformed.SendMessage("CombineMeshNew");
